...
COPY --from=builder /front/client/scripts/revalidate.sh /app/scripts/revalidate.sh

RUN chown nextjs:nodejs /app/scripts/revalidate.sh
RUN chmod +x /app/scripts/revalidate.sh
