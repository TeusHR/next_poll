...
import {ICreateScience, ICreateScienceForm, IScience} from "@/types/Science";
...
const ScienceEdit: FC<Props> = ({scienceId}) => {
    ...
    useEffect(() => {
        setIsLoading(true)
        ScienceService.getScience(scienceId).then(data => setCooperation(data))
            .catch(() => {
    ...
    const onSubmit: SubmitHandler<ICreateScienceForm> = async (dataForm) => {
        ...
        try {
            ...
            ScienceService.updateScience(dataProduct, scienceId, $apiAuth).then((status) => {
                if (status === 200)
