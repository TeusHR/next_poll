...
import {ConferencesService, ScienceCompetitionService} from "@/services/client.service";
...
const ScienceCompetitionID = async ({params}: Params) => {
    const data = await ScienceCompetitionService.get(params.id || '')
            ...
            <div className="flex flex-col gap-14 max-sm:gap-8">
                <div className="flex flec-row gap-8 items-end max-sm:flex-col max-sm:gap-4 max-sm:items-start">
                    ...
                    {<div className="flex flex-row gap-2 sm:min-w-max max-sm:flex-wrap">
                        <span>{data.date} {data.toDate ? ` - ${data.toDate}` : ''}</span>
                        <span className="text-[#D9D9D9]">&#8226;</span>
                        <span>{StringConferenceType(data.type)}</span>
                        <span className="text-[#D9D9D9]">&#8226;</span>
                        <span>{data.country}</span>
                    </div>}
                </div>
                <div className="text-xl" dangerouslySetInnerHTML={{ __html: data.text }}>
...
export default ScienceCompetitionID;
