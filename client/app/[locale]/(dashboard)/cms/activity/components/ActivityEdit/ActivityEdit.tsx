...
import { IActivity, IUpdateActivity, IUpdateActivityForm } from "@/types/Activity";
import revalidateFetch from "@/services/revalidateFetch";
import { filePath } from "../../../consulting/components/ConsultingForm";
...
const ActivityEdit: FC<Props> = ({ activityId }) => {
  const { handleSubmit, control, formState, setValue } = useForm<IUpdateActivityForm>({
    mode: "all",
    defaultValues: {
      title: "",
      text: "",
  ...
  const onSubmit: SubmitHandler<IUpdateActivityForm> = async (dataForm) => {
    ...
    try {
      if (activity) {
        ...
        if (dataForm.image) urlImage = await FileService.upload($apiAuth, dataForm.image, "image");
        ...
        const dataProduct: IUpdateActivity = {
                        ...
                        rules={{
                          validate: async (value) => {
                            if (value && value.length > 0) {
                              ...
                              if (!imagePreview) return "Не вибрано жодного файлу";
                            }
                          },
                        }}
                        render={({ field }) => (
                          <div className="w-full">
                            ...
                            <div className="relative py-2">
                              <div className="border-1 border-primary-500 text-base h-unit-10 rounded-[14px] bg-default-100 hover:bg-default-200">
                                <input
                                ...
                                >
                                  <span>{imagePreview ? imagePreview : "Виберіть файл"}</span>
                                </div>
                              </div>
                              {formState.errors.image?.message && (
                                <div className="text-red-600 text-sm">{formState.errors.image.message}</div>
                              )}
                            </div>
                          </div>
                        )}
