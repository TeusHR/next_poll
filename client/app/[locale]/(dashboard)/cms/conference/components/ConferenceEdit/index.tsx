...
import {IConferences, IUpdateConferences, UpdateConferenceForm} from "@/types/Conference";
...
const ConferenceEdit: FC<Props> = ({conferenceId}) => {
    const {handleSubmit, control, formState, setValue} = useForm<UpdateConferenceForm>({
        mode: "all",
        defaultValues: {
            title: "",
    ...
    useEffect(() => {
        setIsLoading(true);
        ConferencesService.getConference(conferenceId)
    ...
    const onSubmit: SubmitHandler<UpdateConferenceForm> = async (dataForm) => {
        ...
        try {
            ...
            const dataProduct: IUpdateConferences = {
    ...
    useEffect(() => {
        if (conference) {
            ...
            setEditorContent(conference.text);
    ...
    }, [conference, setEditorContent, setValue]);
