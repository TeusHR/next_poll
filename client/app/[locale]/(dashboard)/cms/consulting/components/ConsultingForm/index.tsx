...
type Props = {
  consulting: IConsulting | null | undefined;
  training: ITraining[];
  language: Language;
};
...
const ConsultingFormCreate: FC<Props> = ({ consulting, training, language }) => {
  const { handleSubmit, control, formState, setValue, setError, watch, getValues } = useForm<ICreateConsultingForm>({
    mode: "all",
    defaultValues: {
      title: "",
      text: "",
  ...
  const onSubmit: SubmitHandler<ICreateConsultingForm> = async (dataForm) => {
    ...
    try {
      ...
      const filesPath =
        uploadFiles.length > 0 ? await FileService.upload($apiAuth, FileToFileList(uploadFiles), "image") : [];
  ...
  const createImagesList = (
    filesPath: filePath[],
    dataForm: ICreateConsultingForm,
    serverUploadedFiles: uploadType[],
  ) => {
  ...
  const handleRemoveDynamic = useCallback(
    (fileIndex: number, type: "file" | "image", index: number) => {
  ...
  const handleUploadDynamic = useCallback(
    async (uploadedFiles: File[], type: "file" | "image", index: number) => {
      // if (type === 'image') {
      //     try {
                    ...
                    {fields.map((item, index) => (
                      <div className="flex flex-row gap-4 w-full items-center" key={item.id}>
                        <div className="w-[80px]">
                          <Image src={loadPreviewImage(files[index].file) || files[index].url} alt="preview" />
                        </div>
                        <div className="w-full">
                          <Controller
                        ...
                        <span className="cursor-pointer pt-[20px]" onClick={() => handleRemoveFile(index)}>
                          <CloseIcon />
                        </span>
                      </div>
                    ))}
                  ...
                  <div className="w-full transition flex flex-col gap-4">
                    <div className="flex transition flex-col gap-4 w-full">
                      <div className="flex  w-full items-center justify-between gap-4">
                        <Title text={`Тренування ${idx + 1}`} style="text-[#111318] text-xl max-sm:text-base" />
                        <span className="cursor-pointer" onClick={() => handlerRemoveLab(idx)}>
                          <CloseIcon />
                        </span>
                      </div>
                      <Controller
                        name={`training.${idx}.title`}
                        control={control}
                        rules={{
                          required: "Обов'язкове поле",
                          minLength: { value: 3, message: "Мінімальна довжина 3 символи" },
                      ...
                      <div className="flex flex-col gap-4 w-full relative justify-end">
                        ...
                        <div className="w-full flex flex-col gap-4 items-start">
                          <PreviewUpload
                            files={trainingWatch?.[idx].images || []}
                            type="image"
...
export default ConsultingFormCreate;
