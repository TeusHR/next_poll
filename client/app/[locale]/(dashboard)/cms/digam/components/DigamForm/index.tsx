...
type Props = {
  digam: IDigam | null | undefined;
  language: Language;
};

const DigamFormCreate: FC<Props> = ({ digam, language }) => {
  const { handleSubmit, control, formState, setValue, setError, getValues } = useForm<ICreateDigamForm>({
    mode: "all",
    defaultValues: {
      text: "",
    },
  ...
  useEffect(() => {
    if (digam) {
      ...
      const foreignUniversities: IForeignUniversitiesForm[] = Object.entries(digam.foreignUniversities).map(
  ...
  const onSubmit: SubmitHandler<ICreateDigamForm> = async (dataForm) => {
    ...
    try {
      ...
      const foreignUniversities: ICreateForeignUniversities[] = dataForm.foreignUniversities.flatMap((foreign) =>
        foreign.item.map((item) => {
          return {
                  ...
                  <div className="w-full flex flex-col gap-4">
                    <div className="flex flex-row gap-4 w-full items-center">
                      <div className="w-[80px]">
                        <Image src={loadPreviewImage(files[idx]?.file) || files[idx].url} alt="preview" />
                      </div>
                      <div className="w-full">
                        <Controller
              ...
              <div className="flex transition flex-col gap-4">
                <div className="w-full transition flex flex-col gap-4">
                  <div className="flex transition flex-col gap-6 w-full">
                    ...
                    <NestedFieldArray control={control} formState={formState} parentFieldIndex={idx} />
                  </div>
                </div>
              </div>
            </div>
...
function NestedFieldArray({ control, formState, parentFieldIndex }: NestedFieldArrayProps) {
...
export default DigamFormCreate;
