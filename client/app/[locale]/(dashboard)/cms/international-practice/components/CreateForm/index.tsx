...
import {ICreateInternationalPractice, ICreateInternationalPracticeForm} from "@/types/InternationalPractice";

type Props = {
  language: Language;
};

const InternationalPracticeCreateForm: FC<Props> = () => {
  const { handleSubmit, control, formState, reset, setError } = useForm<ICreateInternationalPracticeForm>({
    mode: "all",
    defaultValues: {
      title: "",
      text: "",
  ...
  const onSubmit: SubmitHandler<ICreateInternationalPracticeForm> = async (dataForm) => {
    ...
    try {
      ...
      const dataProduct: ICreateInternationalPractice = {
      ...
      const status = await InternationalPracticeService.post(dataProduct, $apiAuth);
      if (status === 201) {
...
export default InternationalPracticeCreateForm;
