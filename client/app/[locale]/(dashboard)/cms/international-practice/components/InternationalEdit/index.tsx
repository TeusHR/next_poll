...
import {
    IInternationalPractice,
    IUpdateInternationalPractice,
    IUpdateInternationalPracticeForm
} from "@/types/InternationalPractice";

type Props = {
    itemId: string;
};

const InternationalPracticeEdit: FC<Props> = ({itemId}) => {
    const {handleSubmit, control, formState, setValue, setError} = useForm<IUpdateInternationalPracticeForm>({
        mode: "all",
        defaultValues: {
            title: "",
            text: "",
    ...
    const [internationalPractice, setInternationalPractice] = useState<IInternationalPractice>();
    const [files, setFiles] = useState<uploadType[]>([]);
    const [filesImage, setFilesImage] = useState<uploadType[]>([]);

    useEffect(() => {
        setIsLoading(true);
        InternationalPracticeService.get(itemId)
            .then((data) => setInternationalPractice(data))
            .catch(() => {
    ...
    useEffect(() => {
        if (internationalPractice) {
            setValue("title", internationalPractice.title);
            setValue("text", internationalPractice.text);
            setEditorContent(internationalPractice.text);
            const serverFiles: uploadType[] = internationalPractice.files.map((url) => ({
                name: renderName(url),
            ...
            const serverImage: uploadType[] = internationalPractice.images.map((url) => ({
                name: renderName(url),
                typeUpload: "server" as const,
                type: "image" as const,
                url: url,
    ...
    }, [internationalPractice, setEditorContent, setValue]);

    const onSubmit: SubmitHandler<IUpdateInternationalPracticeForm> = async (dataForm) => {
        ...
        try {
            ...
            const dataProduct: IUpdateInternationalPractice = {
            ...
            const status = await InternationalPracticeService.update(dataProduct, itemId, $apiAuth);
            if (status === 200) {
...
export default InternationalPracticeEdit;
