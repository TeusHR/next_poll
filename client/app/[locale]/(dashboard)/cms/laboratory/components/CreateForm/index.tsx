...
const LaboratoryCreateForm: FC<Props> = ({ language }) => {
  const { handleSubmit, control, formState, reset, getValues, setValue, watch, setError } = useForm<ILaboratoryForm>({
    mode: "all",
    defaultValues: {
      title: "",
      text: "",
  ...
  const { fields, append, remove } = useFieldArray({ control, name: "developments" });
  ...
  const handleUploadDynamic = useCallback(
    (uploadedFiles: File[], type: "file" | "image", index: number) => {
      const newFiles: uploadType[] = uploadedFiles.map((file) => ({
        name: file.name,
        typeUpload: "uploaded" as const,
        type: type,
                  ...
                  <div className="w-full transition flex flex-col gap-4">
                    <div className="flex transition flex-col gap-4 w-full">
                      <div className="flex  w-full items-center justify-between gap-4">
                        <Title text={`Лабораторія ${idx + 1}`} style="text-[#111318] text-xl max-sm:text-base" />
                        <span className="cursor-pointer" onClick={() => handlerRemoveLab(idx)}>
                          <CloseIcon />
                        </span>
                      </div>
                      <Controller
                        name={`developments.${idx}.title`}
                        control={control}
                        rules={{
                          required: "Обов'язкове поле",
                          minLength: { value: 3, message: "Мінімальна довжина 3 символи" },
...
export default LaboratoryCreateForm;
