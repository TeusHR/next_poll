...
const PublicInformationEdit: FC<Props> = ({pageID}) => {
    const {handleSubmit, control, formState, getValues, setValue, watch, setError} = useForm<IPublicInformationForm>({
        mode: "all",
        defaultValues: {
            title: "",
        },
    ...
    const [pageFolders, setPageFolders] = useState<IPublicInformation>();
    const [files, setFiles] = useState<uploadType[]>([]);

    useEffect(() => {
        setIsLoading(true);
        PublicInformation.getPublicInformation(pageID)
            .then((data) => {
                setPageFolders(data);
                console.log(data)
    ...
    const onSubmit: SubmitHandler<IPublicInformationForm> = async (dataForm) => {
        ...
        try {
            ...
            const dataProduct: ICreatePublicInformation = {
                title: dataForm.title,
                language: language.toUpperCase() as Language,
                files: [...existingUrlDocs, ...urlsDocs],
            };

            const status = await PublicInformation.updatePublicInformation(dataProduct, pageID, $apiAuth);
            if (status === 200) {
    ...
    // const isDataChanged = (formData: IPublicInformationForm, initialData?: IPublicInformation): boolean => {
    //     if (!initialData) return true;
    //
    //     console.log(formData, 'formData')
    //     console.log(initialData, 'initialData')
    //
    //     // Проверяем, существует ли files и приводим его к массиву
    //     const formFiles = formData.files
    //         ? Array.from(formData.files).map(file => ('url' in file ? file.url : '') as string).filter(url => url)
    //         : [];
    //
    //     const initialFiles = initialData.files ?? [];
    //
    //     const initialPages = initialData.pages.map(p => ({
    //         title: p.title,
    //         files: p.files,
    //     }));
    //
    //     const formPages = formData.pages?.map(p => ({
    //         title: p.title,
    //         files: p.files ? p.files.map(f => ('url' in f ? f.url : '') as string).filter(url => url) : []
    //     })) ?? [];
    //
    ...
    //
    //
    //     return JSON.stringify({ title: initialData.title, files: initialFiles, pages: initialPages }) !==
    //         JSON.stringify({ title: formData.title, files: formFiles, pages: formPages });
    // };
...
export default PublicInformationEdit;
