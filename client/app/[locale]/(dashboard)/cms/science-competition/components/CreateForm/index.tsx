...
import {CreateScienceCompetitionForm, ICreateScienceCompetition} from "@/types/ScienceCompetition";

type Props = {
    language: Language;
};

const ScienceCompetitionCreateForm: FC<Props> = ({language}) => {
    const {handleSubmit, control, formState, reset, watch} = useForm<CreateScienceCompetitionForm>({
        mode: "all",
        defaultValues: {
            title: "",
    ...
    const onSubmit: SubmitHandler<CreateScienceCompetitionForm> = async (dataForm) => {
        ...
        try {
            ...
            const dataProduct: ICreateScienceCompetition = {
            ...
            const status = await ScienceCompetitionService.post(dataProduct, $apiAuth);
            if (status === 201) {
                handlerReset();
...
export default ScienceCompetitionCreateForm;
