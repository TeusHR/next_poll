...
import { ICreateScienceForm, IScience, IUpdateScience } from "@/types/Science";
...
const ScienceEdit: FC<Props> = ({ scienceId }) => {
  const { handleSubmit, control, formState, setValue } = useForm<ICreateScienceForm>({
    mode: "all",
    defaultValues: {
      title: "",
      text: "",
  ...
  const [cooperation, setCooperation] = useState<IScience>();

  useEffect(() => {
    setIsLoading(true);
    ScienceService.getScience(scienceId)
  ...
  const onSubmit: SubmitHandler<ICreateScienceForm> = async (dataForm) => {
    ...
    setIsLoading(true);
    try {
      const dataProduct: IUpdateScience = {
