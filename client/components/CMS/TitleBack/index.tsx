        ...
        <div className={`group/title w-fit ${isBack ? 'cursor-pointer' : 'cursor-default'} mb-8`}>
            {isBack
                ? <TooltipFD label="Повернутися назад" delay={300}>
                    <div>
                        <button className="flex items-center gap-1 transition-opacity group-hover/title:opacity-40" onClick={() => router.push("/" + pathname.split('/').slice(1, -1).join('/'))}>
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-[30px]">
                                <path
                            ...
                            <span className="text-black font-montserrat text-3xl max-xsm:text-2xl font-bold transition-opacity">
                                {title}
                            </span>
                        </button>
                    </div>
